...
class TryOnPipeline:
    ...
    def _setup_tryon_model(self):
        """Load the TryOn model efficiently for 4GB VRAM."""
        import gc
        model_path = os.path.join(self.weights_dir, "model.safetensors")
        self.logger.info(f"Loading TryOnModel from {model_path} on CPU...")
        ...
        self.logger.info(f"Moving TryOnModel to {self.device} ({self.inference_dtype})...")
